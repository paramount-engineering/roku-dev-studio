...
export const devApp = {
  ...
  screenshotPreviewTitle: 'Zrzut ekranu',

  captureSuccess: 'Przechwycono!',
  rceStreamNotReady: 'Strumień wideo nie jest jeszcze gotowy.',
  rceVideoConnecting: 'Łączenie ze strumieniem wideo…',
  rceVideoReconnecting: 'Ponowne łączenie ze strumieniem wideo…',
  rceVideoStopped: 'Strumień wideo zatrzymany — urządzenie już nie działa.',
  rceVideoError: (detail: string): string => `Błąd strumienia wideo: ${detail}`,
  rceVolumeLabel: 'Głośność',
  rceMuteLabel: 'Wycisz',
  rceUnmuteLabel: 'Wyłącz wyciszenie',
  rceDeviceScreenRelay: 'Przekaz Ekranu',
  rceGalleryButton: 'Zrzuty ekranu',
  rceGalleryTitle: 'Zrzuty Ekranu z Sesji',
  rceGalleryEmpty: 'W tej sesji nie przechwycono jeszcze żadnych zrzutów ekranu.',
  rceGalleryCapturedAt: (time: string): string => `Przechwycono ${time}`,
  ...
  exportCpuDialogTitle: 'Eksportuj Dane Użycia CPU',
  exportMemoryDialogTitle: 'Eksportuj Dane Użycia Pamięci',
  exportObjectsDialogTitle: 'Eksportuj Dane Obiektów BrightScript',
  nothingToExportYet: 'Nie ma jeszcze nic do wyeksportowania',
  exportedTo: (filePath: string): string => `✓ Wyeksportowano do: ${filePath}`,
  exportFailed: (detail: string): string => `Eksport nie powiódł się: ${detail}`,
