...
export const app = {
  ...
  // Add-location flow — aba RCE
  rceNameRequired: 'É necessário um nome.',
  rceTokenRequired: 'É necessário um token.',
  rceAccountExists: (name: string): string => `Já existe uma conta RCE chamada "${name}".`,
  rceTokenExists: (name: string): string => `Este token já foi adicionado como "${name}".`,
  rceUserExists: (name: string): string => `Esta conta RCE já foi adicionada como "${name}".`,
  locationForgetOnQuitTip: 'Esquecida ao sair do app',
  rceNoStoredAccount: (name: string): string => `Nenhuma conta RCE salva chamada "${name}".`,
  rceAccountStatus: 'Conta RCE',

  // RCE device lifecycle — Start/Stop
  rceStart: 'Iniciar',
  rceStop: 'Parar',
  rceStarting: 'Iniciando…',
  rceWakeDevice: 'Ativar dispositivo',
  rceDevMode: 'Modo desenvolvedor',
  rceStatusOff: 'Desligado',
  rceNoStartableSnapshot: 'Nenhum snapshot ou versão de firmware utilizável foi encontrado para este dispositivo.',
  rceNoInstanceUrl: 'O dispositivo não tem uma URL de instância — ele ainda está em execução?',
  rceNoLongerRunning: 'O dispositivo não está mais em execução.',
  rceDevPasswordRequired: 'É necessária uma senha de desenvolvedor.',

  rceStartDeviceOptions: 'Opções',
  rceStartDeviceOptionsAria: 'Opções de inicialização do dispositivo',
  rceStartDeviceModalTitle: (name: string): string => `Iniciar dispositivo RCE: ${name}`,
  rceStartDevice: 'Iniciar dispositivo',
  rceMaxRunTime: 'Tempo máximo de execução',
  rceMaxRunTimeHour: 'Hora',
  rceMaxRunTimeMin: 'Min',
  rceMaxRunTimeHint: 'Até 48 horas',
  rceFirmwareVersion: 'Versão do firmware',
  rceFirmwareSelectPlaceholder: 'Selecionar firmware',
  rceFirmwareNoData: 'Sem dados de firmware',
  rceFirmwareRequired: 'Selecione uma versão de firmware.',
  rceFirmwareRetired: (id: string): string => `O firmware deste dispositivo (${id}) não está mais disponível — selecione um diferente.`,
  rceFirmwareUnavailable: (id: string): string =>
    `O firmware ${id} não está mais disponível para este dispositivo — clique no menu suspenso ao lado de Iniciar para escolher outro.`,
  rceUseThisSnapshot: 'Usar este instantâneo',
  rceSnapshotAria: 'Instantâneo',
  rceSnapshotSelectPlaceholder: 'Selecionar instantâneo',
  rceSnapshotInitialOption: 'Instantâneo inicial',
  rceSnapshotLoadFailed: 'Não foi possível carregar os instantâneos — usando o padrão.',
  rceSnapshotStarted: 'Iniciado',
  rceSnapshotReady: 'Pronto',
  rceSnapshotLive: 'Ao vivo',
  rceSnapshotBase: 'Base',

  // RCE user info modal ("User Info" button, GET /user/me)
  rceCloudEmulatorTitle: 'Roku Cloud Emulator',
  rceUserInfoTitle: 'Informações do usuário',
  rceUserInfoOrganization: 'Organização',
  rceUserInfoName: 'Usuário',
  rceUserInfoEmail: 'E-mail',
  rceUserInfoDevices: 'Dispositivos',
  rceUserInfoSnapshots: 'Instantâneos',
  rceUserInfoMaxRuntime: 'Tempo de execução máximo',
  rceUserInfoDevicesValue: (used: number, max: number): string => `${used} / ${max}`,
  rceUserInfoRuntimeValue: (hours: number): string => `${hours}h`,
  rceUserInfoLoadFailed: 'Falha ao carregar as informações do usuário.',
  rceUserInfoLoading: 'Carregando informações do usuário...',

  rceUsageHeading: 'Uso — últimos 7 dias',
  ...
  rceDeviceInfoTitle: 'Informações do dispositivo',
  rceDeviceInfoCreated: 'Criado',
  rceDeviceInfoLastSnapshot: 'Último instantâneo',
  rceDeviceInfoNote: 'Nota',
  rceDeviceInfoInstanceId: 'ID da instância',
  ...
  serverCapabilities: {
    ...
    screenRelay: { label: 'Retransmissão de tela', desc: 'Transmissão de vídeo ao vivo da tela do dispositivo' },
  ...
  serverVersionTitle: 'Versão do servidor remoto',
  apiVersionTitleHtml: 'Versão do <code>roku-dev-studio-api</code> incluída neste servidor',
...
MAC WiFi: ${d.wifiMac || 'N/A'}`,
  ...
  deviceImageNotFetched: 'Não foi possível obter a imagem do dispositivo.',
  ...
  ecpWarnLimitedDesc:
    'O controle remoto básico e a entrada de texto funcionam, mas algumas consultas de apps e as teclas completas do controle podem não estar disponíveis — defina o Acesso à rede como <strong>Permissivo</strong> ou <strong>Ativado</strong> para a funcionalidade completa.',
  ...
  hideDevicePerformance: 'Ocultar desempenho do dispositivo',
  devicePerformanceToggleTitle: 'Alternar os gráficos de desempenho do dispositivo',
  toggleFps: 'Alternar FPS',
  fpsToggleTitle: 'Alternar o medidor de FPS na tela',
  remoteBack: 'Voltar',
  remoteHome: 'Início',
  ...
  memMetricsMode: 'Modo de métricas de memória',
  memModeSystem: 'Sistema',
  memModeGraphics: 'Gráficos',
  viewBitmapsTitle: 'Ver Bitmaps Gráficos',
  viewBitmapsAria: 'Ver Bitmaps Gráficos',
  legendTotal: 'Total',
  legendUser: 'Usuário',
  legendKernel: 'Kernel',
  legendUsed: 'Usada',
  ...
  chartInfoTitle: 'Informações do Gráfico',
  chartInfoAria: 'Informações do Gráfico',
  chartExportTitle: 'Exportar Dados do Gráfico',
  chartExportAria: 'Exportar Dados do Gráfico',
  ...
  run: 'Executar',
  customQueryPlaceholder: 'ex. /query/device-info ou free',
  results: 'Resultados',
