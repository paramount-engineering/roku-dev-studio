...
export const modals = {
  ...
  features: {
    ...
    console: {
      blurb:
        'Транслюйте наживо вихідні дані налагодження BrightScript з Roku через Telnet (порт 8085), з фільтрацією та пошуком, щоб виявити саме те, що важливо; відкривайте інші консолі пристрою у вікні портів і підключайте повноцінний дебагер BrightScript, коли потрібно покроково пройти код.',
      points: [
        'Живий потік журналу Telnet — консоль налагодження BrightScript на порту 8085',
        ...
        'Вікно портів: консоль SceneGraph 8080, консоль заставки 8087, живе трасування протоколу налагодження 8081 або власний порт (на пристроях Cloud Emulator: 9999 або 49152–65535)',
        'Підключіть дебагер через порт 8081 — точки зупину, змінні, стек викликів і REPL',
      ],
    },
    actionScripts: {
      blurb:
    ...
    remoteLocations: {
      blurb:
        'Підключайтеся до пристроїв Roku, яких немає у вашій локальній мережі — через сервер-ретранслятор в іншому місці або до віртуальних пристроїв Roku у хмарі з облікового запису Roku Cloud Emulator (RCE).',
      points: [
        'Отримуйте доступ до пристроїв LAN в іншому місці через сервер-ретранслятор',
        'Додайте обліковий запис RCE з персональним токеном доступу, щоб переглядати, запускати та керувати його віртуальними пристроями',
  ...
  chartInfo: {
    cpuTitle: 'Використання CPU — Про графік',
    cpuIntro: 'Відсоток використання CPU запущеним застосунком за даними Монітора ресурсів Roku.',
    cpuTotalDesc: 'Сума використання User та Kernel.',
    cpuUserDesc: 'Час CPU, витрачений на власний код застосунку (простір користувача).',
    cpuKernelDesc: 'Час CPU, витрачений у просторі ядра від імені застосунку (системні виклики, введення/виведення тощо).',

    memoryTitle: 'Використання пам’яті — Про графік',
    memoryIntro: 'Режим System — системна пам’ять, яку споживає запущений застосунок.',
    memoryUsedDesc: 'Загальна пам’ять, яку використовує процес застосунку — сума Resident та Swap.',
    memoryResidentDesc: 'Пам’ять, відображена в адресний простір застосунку: анонімна, файлова та спільна пам’ять разом.',
    memoryAnonymousDesc: 'Пам’ять на кшталт стека й купи; також включає виділення demux під час відтворення медіа.',
    memorySharedDesc: 'Спільна пам’ять, яку використовує процес застосунку, напр. буфери медіаплеєра та канали IPC.',
    memoryLimitDesc: 'Ліміт пам’яті переднього плану пристрою — перевищення його, коли застосунок на передньому плані, може призвести до завершення роботи застосунку.',
    graphicsIntro: 'Режим Graphics — пам’ять графічних ресурсів у домені GPU.',
    graphicsTextureDesc: 'Пам’ять графічних ресурсів (r2d2), уже використана GPU.',
    graphicsSystemDesc: 'Пам’ять графічних ресурсів (r2d2), ще не використана GPU — потенційно змарнована пам’ять.',

    objectsTitle: 'Об’єкти BrightScript — Про графік',
    objectsIntro: 'Активні екземпляри об’єктів BrightScript у запущеному застосунку, згруповані за типом (показано топ-10).',
    objectsCountDesc: 'Кількість активних екземплярів кожного типу об’єкта.',
    objectsMemoryDesc: 'Приблизний обсяг пам’яті, у МБ, який споживає кожен тип об’єкта.',

    dataSource: 'Джерело даних:',
    systemNote: '(Система)',
    graphicsNote: '(Графіка)',
    learnMore: 'Докладніше в документації Монітора ресурсів Roku',
  },
