...
export const settings = {
  ...
  mcpToolsButton: 'Переглянути інструменти MCP',
  mcpToolsModalTitle: 'Інструменти MCP',
  mcpToolsModalAria: 'Інструменти MCP, доступні AI-агентам',
  mcpToolsIntro: (count: number): string =>
    `${count} інструментів, які Roku Dev Studio надає AI-агентам через Model Context Protocol, згруповані за областями.`,
  mcpToolCategories: {
    discovery: 'Підключення та виявлення пристроїв',
    remote: 'Дистанційне керування та ECP',
    sideload: 'Сайдлоуд',
    telnet: 'Telnet і консоль',
    appConnector: 'App Connector і RALE',
    debugger: 'Дебагер',
    networkInspector: 'Інспектор мережі',
    actionScripts: 'Скрипти дій',
  },
  mcpToolDescriptions: {
    probe_bridge: 'Перевірити, чи запущений і доступний сам Roku Dev Studio.',
    scan_devices: 'Виявити пристрої Roku в локальній мережі (SSDP + опційне сканування підмережі).',
    list_devices: 'Перелічити всі пристрої, які Dev Studio вже знає — підключені, виявлені або збережені.',
    connect_device: 'Відкрити або сфокусувати вкладку Dev Studio для заданого пристрою.',
    test_connection: 'Перевірити доступність ECP за IP-адресою одного пристрою.',
    get_selected_device: 'Повернути вкладку пристрою, яка наразі активна в Dev Studio.',
    keypress: 'Надіслати одну клавішу дистанційного керування ECP (Вгору / Вниз / Вибрати / Головна / Відтворити / …).',
    input_text: 'Ввести буквальний текст у поточне активне поле введення.',
    launch_app: 'Запустити канал на його головний екран.',
    deep_link: 'Запустити канал прямо на конкретний контент через глибоке посилання ECP.',
    ecp_query: 'Виконати read-only ECP GET — інформація про пристрій, встановлені додатки, активний додаток, стан плеєра, …',
    ecp_post: 'Виконати POST на довільний ECP-ендпоінт (зі побічними ефектами).',
    screenshot: 'Зробити знімок поточного екрана пристрою, повернутий одразу як зображення.',
    get_app_icon: 'Отримати іконку каналу.',
    device_performance_metrics: 'Дані часових рядів ЦП / Пам’яті / Об’єктів BrightScript за запитаний період, з легендою розшифрування.',
    sideload: 'Завантажити та встановити .zip-пакет каналу, замінивши поточний Dev App.',
    delete_sideload: 'Видалити наразі сайдлоуднутий Dev App.',
    telnet_connect: 'Відкрити консоль налагодження BrightScript (порт 8085).',
    telnet_disconnect: 'Закрити консоль налагодження BrightScript.',
    get_telnet_log: 'Прочитати буферизовані рядки консолі, накопичені з моменту підключення.',
    console_monitor_findings: 'Проаналізувати буфер консолі на розпізнані проблеми та збої BrightScript.',
    app_connector_connect: 'Відкрити сесію RALE / App Connector до запущеного Dev App.',
    app_connector_disconnect: 'Закрити сесію RALE / App Connector.',
    app_function: 'Викликати одну функцію каналу через App Connector.',
    list_app_connector_functions: 'Перелічити доступні функції RALE запущеного каналу та їх параметри.',
    rale_command: 'Виконати будь-яку вбудовану команду RALE, включно з деструктивними операціями реєстру.',
    rale_get_node_by_id: 'Read-only отримання одного вузла SceneGraph за id.',
    debugger_attach: 'Відкрити сесію налагодження BrightScript (канал має бути запущений із налагодженням).',
    debugger_status: 'Повернути стан сесії без блокування.',
    debugger_set_breakpoints: 'Додати точки зупину — реєструються негайно, якщо зупинено.',
    debugger_list_breakpoints: 'Перелічити відстежувані точки зупину та їх стан (підтверджено / у черзі).',
    debugger_remove_breakpoints: 'Видалити точки зупину за файлом і рядком.',
    debugger_pause: 'Запросити зупинку запущеного каналу.',
    debugger_continue: 'Продовжити виконання зі зупиненого стану.',
    debugger_step: 'Покроково виконати зупинений потік (over / in / out).',
    debugger_wait_for_stop: 'Блокувати до наступної зупинки, повертаючи повний знімок стану.',
    debugger_get_callstack: 'Повернути кадри стеку викликів для зупиненого потоку.',
    debugger_get_variables: 'Повернути змінні в межах кадру стеку.',
    debugger_evaluate: 'Виконати вираз BrightScript у зупиненому кадрі (REPL).',
    debugger_detach: 'Закрити сесію налагодження.',
    network_inspector_status: 'Повідомити стан захоплення / MITM та передумови налаштування.',
    network_inspector_list_events: 'Перелічити захоплені мережеві події як легкі, фільтровані підсумки.',
    network_inspector_get_event_detail: 'Отримати повні заголовки та тіло для однієї захопленої події.',
    network_inspector_find: 'Повнотекстовий пошук у захоплених URL, заголовках і тілах.',
    network_inspector_analyze: 'Агрегувати буфер у хотспоти та підсумки — статус, хости, типи контенту.',
    network_inspector_get_ca_info: 'Повернути відбиток CA MITM та інформацію про проксі для розшифрування HTTPS.',
    list_action_types: 'Перелічити всі підтримувані типи кроків Action Script.',
    get_action_schema: 'Повернути схему авторства для одного типу кроку.',
    get_capability_bundle: 'Єдиний пакет усіх статичних можливостей — дії, словник, шаблони, контракт авторства.',
    validate_script: 'Перевірити багатокроковий Action Script перед надсиланням у builder.',
    send_script_to_builder: 'Надіслати перевірений Action Script у Dev Studio Builder для перегляду.',
  },
  ...
  caRowDescRemote: 'Центр сертифікації цієї локації, який її проксі використовує для розшифрування HTTPS.',
  ...
  caSectionDescRemote:
    'Інспектор мережі підписує розшифрований HTTPS центром сертифікації цієї локації. Довірте або імпортуйте його на пристрої, щоб ваш канал розробника приймав проксі. Приватний ключ ніколи не залишає той сервер.',
  caSubject: 'Суб’єкт',
