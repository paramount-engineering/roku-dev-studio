...
export const devApp = {
  ...
  screenshotPreviewTitle: 'Captura de pantalla',

  captureSuccess: '¡Capturado!',
  rceStreamNotReady: 'La transmisión de video aún no está lista.',
  rceVideoConnecting: 'Conectando a la transmisión de video…',
  rceVideoReconnecting: 'Reconectando a la transmisión de video…',
  rceVideoStopped: 'Transmisión de video detenida: el dispositivo ya no está en ejecución.',
  rceVideoError: (detail: string): string => `Error de la transmisión de video: ${detail}`,
  rceVolumeLabel: 'Volumen',
  rceMuteLabel: 'Silenciar',
  rceUnmuteLabel: 'Activar sonido',
  rceDeviceScreenRelay: 'Retransmisión de Pantalla',
  rceGalleryButton: 'Capturas',
  rceGalleryTitle: 'Capturas de la Sesión',
  rceGalleryEmpty: 'Aún no se han capturado imágenes en esta sesión.',
  rceGalleryCapturedAt: (time: string): string => `Capturado ${time}`,
  ...
  captionSystemMemory: 'Uso de memoria (Sistema)',
  captionGraphicsMemory: 'Uso de memoria (Gráficos)',
  ...
  skippedNoGraphics: (caption: string): string =>
    `Se omitió la captura de "${caption}" — el dispositivo no devolvió datos de r2d2-bitmaps.`,
  ...
  exportCpuDialogTitle: 'Exportar Datos de Uso de CPU',
  exportMemoryDialogTitle: 'Exportar Datos de Uso de Memoria',
  exportObjectsDialogTitle: 'Exportar Datos de Objetos BrightScript',
  nothingToExportYet: 'Aún no hay nada que exportar',
  exportedTo: (filePath: string): string => `✓ Exportado a: ${filePath}`,
  exportFailed: (detail: string): string => `Error al exportar: ${detail}`,
  ...
  bitmapsModalEmpty: 'Aún no hay datos de bitmaps.',
  bitmapsColName: 'Nombre',
  bitmapsColDimensions: 'Dimensiones',
  bitmapsColBpp: 'BPP',
  bitmapsBppTooltip: 'Bits por píxel',
  bitmapsUpdatedAt: (timeStr: string, ago: string): string => `Actualizado ${timeStr} (${ago})`,
  bitmapsUpdatedAtPlain: (timeStr: string): string => `Actualizado ${timeStr}`,
  bitmapsUpdatedNever: 'Aún no actualizado',
  bitmapsAgoSeconds: (s: number): string => `hace ${s}s`,
  bitmapsAgoMinutes: (m: number): string => `hace ${m}m`,
  bitmapsColSize: 'Tamaño',
