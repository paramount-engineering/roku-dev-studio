 ...
 *  - a Save footer action (the header close button covers cancel).
 *
    ...
    setSafeHTML(
      ...
      `<div class="modal action-scripts-save-modal" role="dialog" aria-modal="true" aria-label="${escapeHtml(S.actionScripts.saveActionScriptBtn)}">
         <div class="modal-header">
           ...
           <button type="button" class="modal-close action-scripts-save-modal-close" title="${escapeHtml(S.common.close)}" aria-label="${escapeHtml(S.common.close)}"><span class="icon icon-sm"><svg><use href="#icon-x"/></svg></span></button>
         </div>
